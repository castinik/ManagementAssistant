...
namespace ManagementAssistant.Data
{
    public class APIManager
    {
        ...
        Processing _processing = new Processing();
        ...
        public String GetCurrentValue(String market, String symbol = "", String from_currency = "", String to_currency = "")
        {
            ...
            try
            {
                ...
                {
                    case "stock":
                        currentValue = _apiMarkets.GetStockCurrentValue(symbol);
                        break;
                    case "crypto":
                        currentValue = _apiMarkets.GetCryptoForexCurrentValue(from_currency);
                        break;
                    case "forex":
                        currentValue = _apiMarkets.GetCryptoForexCurrentValue(from_currency, to_currency);
                        break;
                    default:
                        break;
                }
                _dbContext.UpdateCurrentValue(market, currentValue, symbol, from_currency, to_currency);
            }
            catch
            {
        ...
        public void GetDailyStock(String symbol)
        {
            try
            {
                ...
                stockModel.StockMetaDataModel.CurrentValue = _apiMarkets.GetStockCurrentValue(symbol);
        ...
        #region ##### PRIVATE GET METHOD FOR DBCONTEXT #####
        #endregion
        ...
        private String SetMarketValue(String market)
        {
            String valueSql = String.Empty;
            switch (market)
            {
                case "stock":
                    valueSql = $"SELECT [Information] FROM [dbo].[_Stocks] WHERE Symbol = ";
                    break;
                case "crypto":
                    valueSql = "SELECT [DigitalCurrencyName] FROM [dbo].[_Cryptocurrencies] WHERE DigitalCurrencyCode = ";
                    break;
                case "forex":
                    valueSql = "SELECT [FromSymbol], [ToSymbol] FROM [dbo].[_Forex]";
                    break;
                default:
                    break;
            }
